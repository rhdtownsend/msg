...
of a wavelength value followed by a flux value. Blank lines are
ignored, and comments and label definitions are supported via
the :option:`--comment-prefix` and :option:`--label-prefix` options.
...
.. option:: --comment-prefix=PREFIX

   Treat lines prefixed by PREFIX as comments.

.. option:: --label-prefix=PREFIX

   Treat lines prefixed by PREFIX as photospheric parameter
   definitions (formatted as NAME:VALUE).

.. option:: -l, --label=NAME:VALUE

   Photospheric parameter name and value. Can be specified multiple
   times, to define multiple parameters. Overrides values defined
   using :option:`--label-prefix`.
